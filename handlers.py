...
from parsing import build_bot_handle, build_passage_from_ref, command_list, ensure_text, other_version, parse_get_request
from state import (
    BACK_TO_LANGUAGES,
    DEFAULT_VERSION,
    EMPTY,
...
def get_try_inline_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [[InlineKeyboardButton("Try inline mode", switch_inline_query_current_chat="john 3:16 nlt")]]
    )
...
def get_default_version(context: CallbackContext) -> str:
    return ensure_text(context.user_data.get(USER_VERSION_KEY) or DEFAULT_VERSION).upper()
...
async def get_conversation_message(
    ...
    version = ensure_text(context.user_data.pop("pending_get_version", get_default_version(context)))
    display_name, _, _ = get_identity(update)
...
async def search_command_entry(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    raw_text = ensure_text(update.effective_message.text).strip()
    ...
    display_name, _, _ = get_identity(update)
    await reply_with_search_results(update, context, parts[1].strip().lower(), display_name)
    return ConversationHandler.END
    ...
    if len(parts) > 1:
        ...
        if version not in VERSIONS:
            await update.effective_message.reply_text(
                ...
                "Current default is {}.".format(display_name, get_default_version(context))
            )
            return ConversationHandler.END
        set_default_version(context, version)
        await update.effective_message.reply_text(f"Success! Default version is now {version}.")
        return ConversationHandler.END
...
async def linked_passage_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    raw_text = ensure_text(update.effective_message.text).strip()
    display_name, _, _ = get_identity(update)
...
async def quick_lookup_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    raw_text = ensure_text(update.effective_message.text).strip()
    lowered = raw_text.lower()
    bot_handle = build_bot_handle(context.application).lower()

    if is_group_chat(update) and bot_handle not in lowered and not replied_to_bot(update):
