...
async def close_http_client(application: Application) -> None:
    client: BibleGatewayClient = application.bot_data["bible_client"]
    await client.close()
...
def build_application() -> Application:
    ...
    get_conversation = ConversationHandler(
        entry_points=[MessageHandler(filters.Regex(r"^/get(\w+)?(@\w+)?(\s|$)"), get_command_entry)],
        states={
            GET_PASSAGE_STATE: [MessageHandler(filters.TEXT & ~filters.COMMAND, get_conversation_message)]
        },
    ...
    search_conversation = ConversationHandler(
        ...
        states={
            SEARCH_STATE: [MessageHandler(filters.TEXT & ~filters.COMMAND, search_conversation_message)]
        },
        fallbacks=[CommandHandler("cancel", cancel_conversation)],
    ...
    setdefault_conversation = ConversationHandler(
        entry_points=[
            ...
            MessageHandler(filters.Regex(r"^/start setdefault$"), start_setdefault_entry),
        ],
        states={
            SETDEFAULT_LANGUAGE_STATE: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, setdefault_language_message)
            ],
            SETDEFAULT_VERSION_STATE: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, setdefault_version_message)
            ],
        },
        fallbacks=[CommandHandler("cancel", cancel_conversation)],
    ...
    application.add_handler(CommandHandler("botfamily_verification_code", botfamily_verification_command))
    ...
    application.add_handler(MessageHandler(filters.Regex(r"^/(" + "|".join(BOOKS) + r")"), linked_passage_handler))
    application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, quick_lookup_handler))
