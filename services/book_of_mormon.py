...
BOOK_OF_MORMON_BASE_URL = "https://www.churchofjesuschrist.org/study/scriptures/bofm"
BOOK_OF_MORMON_VERSION = "BOM"


@dataclass(frozen=True)
class BookOfMormonBook:
    title: str
    slug: str
    path: str
    chapters: int
...
BOOK_OF_MORMON_BOOKS: tuple[BookOfMormonBook, ...] = (
    BookOfMormonBook(
        "1 Nephi", "1nephi", "1-ne", 22, ("1 nephi", "1 ne", "1nephi", "1ne")
    ),
    BookOfMormonBook(
        "2 Nephi", "2nephi", "2-ne", 33, ("2 nephi", "2 ne", "2nephi", "2ne")
    ),
    BookOfMormonBook("Jacob", "jacob", "jacob", 7, ("jacob", "jac")),
    BookOfMormonBook("Enos", "enos", "enos", 1, ("enos",)),
    BookOfMormonBook("Jarom", "jarom", "jarom", 1, ("jarom",)),
    BookOfMormonBook("Omni", "omni", "omni", 1, ("omni",)),
    BookOfMormonBook(
        "Words of Mormon",
        "wordsofmormon",
        "w-of-m",
    ...
    BookOfMormonBook("Mosiah", "mosiah", "mosiah", 29, ("mosiah", "mos")),
    BookOfMormonBook("Alma", "alma", "alma", 63, ("alma",)),
    BookOfMormonBook("Helaman", "helaman", "hel", 16, ("helaman", "hel")),
    BookOfMormonBook(
        "3 Nephi", "3nephi", "3-ne", 30, ("3 nephi", "3 ne", "3nephi", "3ne")
    ),
    BookOfMormonBook(
        "4 Nephi", "4nephi", "4-ne", 1, ("4 nephi", "4 ne", "4nephi", "4ne")
    ),
    BookOfMormonBook("Mormon", "mormon", "morm", 9, ("mormon", "morm")),
    BookOfMormonBook("Ether", "ether", "ether", 15, ("ether", "eth")),
    BookOfMormonBook("Moroni", "moroni", "moro", 10, ("moroni", "moro", "mor")),
)

BOOK_OF_MORMON_BOOK_BY_SLUG = {book.slug: book for book in BOOK_OF_MORMON_BOOKS}


@dataclass(frozen=True)
class BookOfMormonReference:
    book: BookOfMormonBook
    start_chapter: int
...
def normalize_book_of_mormon_name(name: str) -> str:
    return re.sub(r"[^a-z0-9]+", "", name.lower())


BOOK_OF_MORMON_BOOK_BY_ALIAS = {
    normalize_book_of_mormon_name(alias): book
    for book in BOOK_OF_MORMON_BOOKS
    for alias in book.aliases
}


def format_reference_title(reference: BookOfMormonReference) -> str:
    if (
...
def parse_book_of_mormon_reference(passage: str) -> BookOfMormonReference | None:
    ...
    if match is None:
        ...
        book_name = normalize_book_of_mormon_name(chapter_match.group(1))
        book = BOOK_OF_MORMON_BOOK_BY_ALIAS.get(book_name)
        if book is None:
            return None
        ...
        if not (1 <= start_chapter <= end_chapter <= book.chapters):
            return None
        return BookOfMormonReference(book, start_chapter, None, end_chapter, None)

    book_name = normalize_book_of_mormon_name(match.group(1))
    book = BOOK_OF_MORMON_BOOK_BY_ALIAS.get(book_name)
    if book is None:
        return None
...
def parse_passage_html(
    ...
    *,
    ...
    header = build_passage_header(
        format_reference_title(reference), BOOK_OF_MORMON_VERSION
    )
    ...
    return InlinePassageResult(
        passage=final_text,
        result_id=f"{reference.book.slug}.{reference.start_chapter}/{BOOK_OF_MORMON_VERSION}",
        title=header,
...
class BookOfMormonClient:
    def __init__(self, client=None):
        if httpx is None:
            raise RuntimeError("httpx is required to use BookOfMormonClient.")
    ...
    async def _get_chapter_html(
        self, book: BookOfMormonBook, chapter: int
    ) -> str | None:
        url = f"{BOOK_OF_MORMON_BASE_URL}/{book.path}/{chapter}?lang=eng"
        return await self.fetch_text(url)

    async def get_passage(
        self,
        passage: str,
        version: str = BOOK_OF_MORMON_VERSION,
        inline_details: bool = False,
    ) -> str | InlinePassageResult | None:
        reference = parse_book_of_mormon_reference(passage)
        if reference is None:
            return EMPTY

        chapter_outputs: list[str] = []
        for chapter in range(reference.start_chapter, reference.end_chapter + 1):
            chapter_reference = BookOfMormonReference(
                book=reference.book,
