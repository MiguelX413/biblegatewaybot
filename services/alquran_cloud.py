...
QURAN_REFERENCE_PREFIX_PATTERN = re.compile(
    r"(?i)^\s*(?:(?:qur(?:an|['’ʾ]an)|al(?:[\s-]+)qur(?:an|['’ʾ]an)|koran|surah)\s+)?"
)
...
def _parse_named_quran_reference(passage: str) -> QuranReference | None:
    ...
    for alias, surah in sorted(
        QURAN_SURAH_ALIAS_TO_NUMBER.items(),
        key=lambda item: len(item[0]),
        reverse=True,
    ):
        alias_pattern = re.sub(r"(?<=\w)(?=\W)|(?<=\W)(?=\w)", "", alias)
        del alias_pattern
